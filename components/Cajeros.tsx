        ...
        <table className="min-w-full divide-y divide-gray-700">
          ...
          <tbody className="bg-gray-800/50 divide-y divide-gray-700">
            {filteredCajeros.map((cajero) => (
              <tr key={cajero.id} className={`${cajero.estadolinea ? 'bg-green-500/10' : 'bg-red-500/10'} hover:bg-gray-700/50 transition-colors`}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{cajero.nombre}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${cajero.estadolinea ? 'bg-green-500/30 text-green-200' : 'bg-red-500/30 text-red-200'}`}>
